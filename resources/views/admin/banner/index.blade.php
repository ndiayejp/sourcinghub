    ...
    <div class="container-fluid">
        <div class="block-header">
            <a class="btn btn-primary waves-effect" href="{{ route('admin.banner.create') }}">
                ...
                <span>{{ __('Ajouter une bannière') }}</span>
            </a>
        </div>
        <!-- Exportable Table -->
                                ...
                                <tbody>
                                    @foreach($banners as $key=>$banner)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $banner->name }}</td>
                                            <td> <img src="/banners/{{  $banner->image }}" width="50px" height="100px"/></td> 
                                            <td>{{ $banner->end_at }}</td>
                                             <td>
                                                @if($banner->active == 1)
                                                    <span class="badge bg-green">{{ __('Publié') }}</span>
                                                @else
                                                    <span class="badge bg-pink">{{ __('Non publié ') }}</span>
                                                @endif
                                            </td>
